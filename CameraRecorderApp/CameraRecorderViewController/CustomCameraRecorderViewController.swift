...
class CustomCameraRecorderViewController: BasePage, NextLevelDelegate, NextLevelVideoDelegate {
    var item: AttributeItem!
    var completion: ((TestStatus) -> Void)?
    ...
    private var layoutConstraintsSetup = false // 标记是否已经设置过 SnapKit 约束
    ...
    override func viewDidLayoutSubviews() {
        ...
        if let playerLayer = playerLayer, let videoPlaybackContainerView = videoPlaybackContainerView {
            ...
            playerLayer.frame = CGRect(
                ...
                height: contentFrame.height - Constant.screenSafeInset.bottom
            )
        }
    }
    ...
    @objc private func cancelButtonTapped() {
        ...
        // 执行回调
        if item.originalSelectedData.count > 0 {
            completion?(.close(item.originalSelectedData))
        } else {
    ...
    @objc private func useVideoButtonTapped() {
        ...
        playerLayer.frame = CGRect(
            ...
            height: contentView.frame.height - Constant.screenSafeInset.bottom
        )
        ...
        // 弹出 CommonViewController
        let page = CommonViewController()
        page.itemWrapper = AttributeWrapper.init(item: item, initBlock: nil, deinitBlock: nil, systemStatus: .notSystemChecked)
        page.completion = { [weak self] status in
            // 在页面退出前完全清理播放资源
